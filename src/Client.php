	...
	*/
	...
	public function request($callback) {
		...
		*/
		...
		if (isset($this->options['inputFetch'])) {
			...
			$data = json_encode([
				...
				'process' => isset($this->options['process']) ? $this->options['process'] : [],
			]);
		...
		*/
		...
		$this->curlSetopts('POST', isset($this->options['api_url']) ? $this->options['api_url'] : '/image/process', isset($data) ? $data : [], $requestHeaders);
